...
export function ParallelComparison({
  ...
  const loadSecondaryVerse = async () => {
    ...
    try {
      ...
      // Set error message instead of leaving it blank
      setSecondaryText('Parallel version not available')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div
      className={`fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-5xl z-40 transition-all duration-300 parallel-comparison-container ${
              ...
              {loading ? (
                ...
                >
                  {hasStrongs(secondaryVersion) && secondaryText && onStrongsClick ? (
                    (() => {
                      console.log('🔍 [ParallelComparison] About to render VerseWithStrongs with text:', secondaryText.substring(0, 100));
                      return (
                        <VerseWithStrongs
                          text={secondaryText}
                          verseNumber={verse}
                    ...
                    <span>
                      ...
                      <span dangerouslySetInnerHTML={{
                        __html: processBibleText(secondaryText) || 'Loading verse text...'
                      }} />
                    </span>
                  )}
                </div>
